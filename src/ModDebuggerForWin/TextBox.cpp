...
HWND CreateTextBox(HWND hWnd, HINSTANCE hInst ) {
	...
	hEdit = CreateWindowEx(WS_EX_CLIENTEDGE,
		...
		iClientWidth-TextBoxPosX*2, iClientHeight-TextBoxPosY-TextBoxPosX, //とりあえず幅、高さ０のウィンドウを作る
		hWnd,
		(HMENU)1,
		hInst,
		NULL);
...
void MoveTextBox(HWND hWnd) {
	...
	// 移動
	MoveWindow(hEdit, TextBoxPosX, TextBoxPosY, iClientWidth-TextBoxPosX*2, iClientHeight-TextBoxPosY-TextBoxPosX, true);
}
...
BOOL ReWriteAllRichEdit(HWND hEdit) {
	...
	for ( string line : AllRecievedLog) {
		// 正規表現のところ中身空なら全部だす。
